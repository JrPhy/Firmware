...
#### 1. 進入點
u-boot.lds 中的 ENTRY(_start) 就是 U-boot 的進入點，其中 _start 放在 ```u-boot/arch/arm/lib/vectors.S``` 或是每個 ```cpu/start.S``` 中，裡面可看到一開始先做了 reset，之後就到 ```.globl _start```，裡面的 ```.section ".vectors", "ax"```．\
